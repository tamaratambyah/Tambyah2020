...
D(i) = 1/(2*s_old*dz)*(-1/q(i)^2*(k(i)*(1/q(i)-a(i)) - k(i-1)*(1/q(i-1)-a(i-1))) ...
    ...
    +(k(i)/4*(-1/q(i)^2*(1/q(i)*(a(i)-a(i-1)) - 1/q(i-1)*(a(i)-a(i-1)) ) + 1/q(i)*(a(i)-a(i-1))*-1/q(i)^2) ...
    + 1/(4)*(a(i)-a(i-1))*(k(i)-k(i-1))*-2/q(i)^3 ...
    + a(i)/4*(-1/q(i)^2*(1/q(i)*(k(i)-k(i-1)) - 1/q(i-1)*(k(i)-k(i-1)) ) + 1/q(i)*(k(i)-k(i-1))*-1/q(i)^2) )*1/(s_old^2*dz^2);
L(i) = 1/(2*q(i)*s_old*dz)*k(i-1)/q(i-1)^2 ...
    + (k(i)/(4*q(i))*-(a(i)-a(i-1))*-1/q(i-1)^2 + a(i)/(4*q(i))*-(k(i)-k(i-1))*-1/q(i-1)^2)*1/(s_old^2*dz^2);
