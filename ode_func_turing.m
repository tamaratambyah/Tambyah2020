...
f(1) = 0; % fixed boundary at x = 0 from Equation (2.3)
for i = 2:N
    f(i) = 1/eta*(k(i)*(x(i+1) - x(i) - a(i)) - k(i-1)*(x(i) - x(i-1) - a(i-1)));
    % linear force law in Equation (2.1) 
end
f(N+1) = -1/eta*k(N)*(x(N+1)-x(N)-a(N)); % free boundary at x = L(t) from Equation (2.3)

%%% ODES FOR CHEMICAL 1
% internal cells from Equation (2.9)
for i = N+3:2*N
    j = i - (N+1);
    h = i + N;
...
% boundary cell 1 from Equation (2.8)
i = N+2;
j = i - (N+1);
h = i + N;
f(i) = (n1 - n2*x(i) + n3*x(i)^2*x(h)).....
...
% boundary cell N from Equation (2.10)
i = 2*N+1;
j = i - (N+1);
h = i + N;
f(i) = (n1 - n2*x(i) + n3*x(i)^2*x(h))...
